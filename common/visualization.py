...
def render_animation(keypoints, keypoints_metadata, poses, skeleton, fps, bitrate, azim, output, viewport,
    ...
    """
    plt.ioff()  #用于matplotlib可视化交互模式界面停留
    ...
    if downsample > 1:
        ...
        fps /= downsample
    #np.array(poses).shape=(1, 726, 17, 3)
    initialized = False
    image = None
    lines = []
    points = None
    ...
    def update_video(i):
        ...
        for n, ax in enumerate(ax_3d):
            ax.set_xlim3d([-radius/2 + trajectories[n][i, 0], radius/2 + trajectories[n][i, 0]])  #设置坐标范围
        ...
        if not initialized:
            ...
            for j, j_parent in enumerate(parents):
                ...
                col = 'red' if j in skeleton.joints_right() else 'black' #左边关节点黑色，右边关节点是红色
                for n, ax in enumerate(ax_3d):
                    pos = poses[n][i]   #np.array(pos).shape=(17,3)  n取值1至17
                    # print("####n", n)
                    # print("@@@j=", j)
                    # print("$$$$$j_parent=", j_parent)
