...
def read_camera(save_path='output.mp4'):
    w, h = 640, 480  # Set desired width and height for the camera stream

    # ffmpeg command to capture from macOS camera using AVFoundation
    command = [
        'ffmpeg',
        '-f', 'avfoundation',  # Use AVFoundation for macOS
        '-framerate', '30',    # Set frame rate
        '-video_size', f'{w}x{h}',
        '-i', '0',             # Camera device index (0 is typically the default camera)
        '-f', 'tee',           # Tee format to duplicate output
        '-map', '0',           # Map stream 0 (input camera stream)
        '-c:v', 'libx264',     # Set codec for saving video
        f'[f=rawvideo]pipe:1|{save_path}'  # Output: raw video pipe for processing and file for saving
    ]
    ...
    while frames < 60:
        ...
        frame = np.frombuffer(data, dtype='uint8').reshape((h, w, 3))
        frames += 1
        yield frame
...
def main(args):
    ...
    for video_name in im_list:
        ...
        print(boxes)
        saves = {'out_name': out_name, 'boxes':boxes, 'segments':segments, 'keypoints':keypoints, 'metadata':metadata}
        pickle.dump(saves, open('output_directory/videoplayback.obj', 'wb'))
